        ...
        # Comparison
            ...
            if len(tokens) == 3:
                if tokens[2].startswith('$'):
                    rhs_is_var = True
                    rhs_var_name = tokens[2][1:]
                elif tokens[2] in self.variables:
                    rhs_is_var = True
                    rhs_var_name = tokens[2]
                else:
                    value = int(tokens[2])
            ...
            if var_info['type'] != 'int' or var_info['size'] != 8:
                raise NotImplementedError("Comparisons currently supported only for 8-byte 'int' variables")

            var_pos = var_info['pos']

            if rhs_is_var:
                if rhs_var_name not in self.variables:
                    raise ValueError(f"Unknown variable: {rhs_var_name}")
                rhs_info = self.variables[rhs_var_name]
                if rhs_info['type'] != 'int' or rhs_info['size'] != 8:
                    raise NotImplementedError("Comparisons currently supported only for 8-byte 'int' variables")
                rhs_pos = rhs_info['pos']
...
from bfpp.compiler import BrainFuckPlusPlusCompiler as _BrainFuckPlusPlusCompiler

BrainFuckPlusPlusCompiler = _BrainFuckPlusPlusCompiler

__all__ = ['BrainFuckPlusPlusCompiler']
